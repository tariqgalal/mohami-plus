...
import { Plus, Trash2, AlertCircle, FileText, X as XIcon } from "lucide-react";
...
export function CaseForm({ initial, mode }: CaseFormProps) {
  ...
  useEffect(() => {
    // إذا اختار محامي مساعد أنه أصبح الرئيسي، احذفه من المساعدين
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <Card>
        ...
        <CardContent className="grid md:grid-cols-2 gap-4">
          <div className="md:col-span-2 space-y-2">
            ...
            <Input id="title" placeholder="مثال: نزاع تجاري - عقد توريد" {...register("title")} />
            {errors.title && (
      ...
      <Card>
        <CardHeader>
          <CardTitle>العميل والمحامون</CardTitle>
        </CardHeader>
        <CardContent className="grid md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500">
              لا يوجد عميل؟{" "}
              <a href="/dashboard/clients/new" className="text-brand-600 hover:underline">
                أضف عميل جديد
              </a>
            </p>
          </div>
